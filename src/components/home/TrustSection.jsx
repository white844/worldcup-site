...
export default function TrustSection() {
  ...
  const TRUST_CARDS = [
    { icon: "🔒", titleKey: "trust.card1.title", bodyKey: "trust.card1.body", statKey: "trust.card1.stat", lblKey: "trust.card1.lbl" },
    { icon: "🛡️", titleKey: "trust.card2.title", bodyKey: "trust.card2.body", statKey: "trust.card2.stat", lblKey: "trust.card2.lbl" },
    { icon: "✅", titleKey: "trust.card3.title", bodyKey: "trust.card3.body", statKey: "trust.card3.stat", lblKey: "trust.card3.lbl" },
  ];
    ...
    <section id="trust" style={{ background: C.bg, padding: "clamp(40px,8vw,72px) 0" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 clamp(16px,5vw,20px)" }}>
        ...
        <div style={{ textAlign: "center", maxWidth: 600, margin: "0 auto 32px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: C.blue, marginBottom: 8, ...dm }}>{t("trust.label")}</div>
          ...
          {TRUST_CARDS.map(card => (
            <div key={card.titleKey} className="wc26-card" style={{ padding: "clamp(16px,4vw,28px) clamp(14px,3vw,24px)", boxShadow: C.shadowSm, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
                <div style={{ width: 46, height: 46, borderRadius: 12, background: `linear-gradient(135deg,${C.blue},${C.blueDark})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0, boxShadow: C.shadowBlue }}>{card.icon}</div>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, flexWrap: "wrap" }}>
            {PAYMENT_METHODS.map(p => (
              <div key={p} className="wc26-card" style={{ padding: "7px 14px", fontSize: 12, fontWeight: 700, color: C.textMid, boxShadow: C.shadowSm, borderRadius: 8, ...dm }}>{p}</div>
            ))}
          </div>
        </div>
      </div>
